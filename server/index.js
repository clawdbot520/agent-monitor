...
app.get('/api/system/metrics', (req, res) => {
  ...
  setTimeout(() => {
    try {
      ...
      // macOS: os.freemem() counts file cache as free, giving inaccurate results.
      // Use vm_stat to get real usage: app + wired + compressed pages.
      ...
      try {
        if (process.platform === 'darwin') {
          // df / on macOS APFS only reads the sealed System snapshot (~12 GB), not user data.
          // Use diskutil apfs list to get container-level stats (accurate total).
...
// Resolve memory dir for an agent: checks agent dir, then workspace dir from openclaw.json
const getMemoryDir = (agentsDir, agentId) => {
  // Try agent-local memory first
  const agentMem = path.join(agentsDir, agentId, 'memory');
  if (fs.existsSync(agentMem)) return agentMem;

  // Try to get workspace from openclaw.json
  try {
    const home = process.env.HOME || process.env.USERPROFILE;
    const ocPath = path.join(home, '.openclaw', 'openclaw.json');
  ...
  // Fallback: conventional workspace path
  const home = process.env.HOME || process.env.USERPROFILE;
  const wsRoot = path.dirname(agentsDir); // ~/.openclaw
...
// GET /api/agents/:agentId/memory — list memory files
...
// GET /api/agents/:agentId/memory/:filename — read a memory file
...
app.delete('/api/agents/:agentId/sessions/:sessionId', (req, res) => {
  try {
...
// Files live in the workspace dir, NOT in agentDir (which only has auth JSON files).
...
// GET /api/agents/:agentId/config-files — list .md files
app.get('/api/agents/:agentId/config-files', (req, res) => {
  try {
    const { agentId } = req.params;
    const { dir } = req.query;
    const agentsDir = getAgentsDir(dir);
...
// GET /api/agents/:agentId/config-files/:filename — read file
app.get('/api/agents/:agentId/config-files/:filename', (req, res) => {
  try {
    const { agentId, filename } = req.params;
    const { dir } = req.query;
    const agentsDir = getAgentsDir(dir);
...
// PUT /api/agents/:agentId/config-files/:filename — write file
app.put('/api/agents/:agentId/config-files/:filename', (req, res) => {
  try {
    const { agentId, filename } = req.params;
    const { dir } = req.query;
...
// GET /api/lancedb/memories?scope=&category=&limit=30&offset=0
app.get('/api/lancedb/memories', async (req, res) => {
  try {
...
// GET /api/lancedb/search?q=&scope=
app.get('/api/lancedb/search', async (req, res) => {
  try {
...
app.delete('/api/lancedb/memories/:id', async (req, res) => {
  try {
    const table = await getLanceTable()
...
// PATCH /api/lancedb/memories/:id — update text/category (read-delete-reinsert to preserve vector)
app.patch('/api/lancedb/memories/:id', async (req, res) => {
  try {
...
app.post('/api/chat/send', (req, res) => {
  ...
  if (mentionMatch) {
    ...
    if (agent === '小歐') {
      ...
      // Use --form-string to avoid curl treating @ as file path
